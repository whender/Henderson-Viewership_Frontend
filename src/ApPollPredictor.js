  ...
  return <section className="home-panel ap-predictor">
    ...
    {view === 'model' && <>
      ...
      <div className="cfb-week-stats"><div className="cfb-week-stats-heading"><h4>Historical validation · 2019–2025</h4><span>{stats.polls} in-season polls</span></div><div className="cfb-week-metrics">
        <div><span>Top 25 membership</span><strong>{pct(stats.overlap)}</strong><small>{data.baselineLabel || 'Previous-poll baseline'}: {pct(stats.baselineOverlap)}</small></div>
        <div><span>Average rank error</span><strong>{stats.rankError.toFixed(2)}</strong><small>{data.baselineLabel || 'Previous-poll baseline'}: {stats.baselineRankError.toFixed(2)}</small></div>
      ...
      <p className="cfb-footnote">Each validation season was predicted using a model trained only on prior seasons. Top 25 membership is the fraction of actual ranked teams included in the predicted list. Rank error measures actual ranked teams, with unranked predictions counted as 26. {data.modelVersion ? 'The baseline is the earlier AP-tier and Vegas regression. Model selection used 2019–2024; 2025 is a previously examined follow-up.' : 'The baseline keeps the previous poll order.'}</p>
      ...
      {data.drivers.length > 0 && <><h4 className="cfb-schedule-title">What the model learns</h4><p className="cfb-footnote ap-note">Largest fitted effects per one historical standard deviation. These are associations, not causal explanations; correlated factors can share or offset influence.</p>
      ...
      {data.dropValidation && <p className="cfb-notice">Large drops after wins · 2019–2024: detected {data.dropValidation.caughtBigDrops} of {data.dropValidation.actualBigDrops}, with {data.dropValidation.falseAlarms} false alarms among {data.dropValidation.winningTop10Cases - data.dropValidation.actualBigDrops} other winning Top 10 cases. This remains a difficult, rare outcome.</p>}
