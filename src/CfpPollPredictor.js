...
import { eligibleGames, projectScenario, averageScenarios } from './cfpScenario.mjs';
...
export default function CfpPollPredictor() {
  ...
  const projected = useMemo(() => {
    ...
    return data.next.scenarioEngine && Object.keys(outcomes).length ? (simulationIndex === 0 ? averageScenarios(data.next.scenarioEngine, outcomes, data.next.eligibleTeams || []) : projectScenario(data.next.scenarioEngine, simulationIndex, outcomes, data.next.eligibleTeams || [])) : saved;
  ...
  return <section className="home-panel ap-predictor">
    ...
    {view === 'outlook' && <>
      ...
      <p className="cfb-notice">{basis === 'today' ? 'Uses completed games available now, through the final CFP release. Rankings will change as teams build their resumes.' : `${simulationIndex === 0 ? 'Average projection shows mean ranks and records across 25 simulations, ordered by average rank.' : 'Each projection samples game winners using the football model’s probabilities.'} Each individual projection uses a consistent set of results for ${projected.assumptions.length} remaining games. Forced outcomes apply to every projection. In-progress games use saved pregame probabilities.`}</p>
      {basis === 'projected' && <>
        <div className="ap-scenario-actions cfp-projection-controls">
          <button type="button" className={simulationIndex === 0 ? 'active' : ''} aria-pressed={simulationIndex === 0} onClick={() => selectSimulation(0)}>Average projection</button>
          <div className="cfp-projection-stepper">
            ...
            <span role="status" aria-live="polite">{simulationIndex === 0 ? 'Average of 25 projections' : `Projection ${simulationIndex} of ${scenarios.length - 1}`}{Object.keys(outcomes).length > 0 ? ' · Custom outcomes' : ''}</span>
      ...
      {rows.length ? <div className="overflow-x-auto"><table className="cfb-table ap-table"><thead><tr><th>Predicted</th><th>Team</th><th>Record</th><th>Quality wins</th><th>Best wins</th><th>Worst losses</th></tr></thead><tbody>{rows.map(t => <tr key={t.key}><td className="cfb-rank">{t.rank}</td><td><Team row={t} /></td><td className="cfb-number">{projected?.averaged && basis === 'projected' ? t.wins.toFixed(1) : t.wins}–{projected?.averaged && basis === 'projected' ? t.losses.toFixed(1) : t.losses}</td><td>{projected?.averaged && basis === 'projected' ? t.qualityWins.toFixed(1) : t.qualityWins}</td><td>{basis === 'projected' && projected.averaged ? '—' : <Opponents teams={t.bestWins} label={`${t.team} best wins`} />}</td><td>{basis === 'projected' && projected.averaged ? '—' : <Opponents teams={t.worstLosses} label={`${t.team} worst losses`} />}</td></tr>)}</tbody></table></div> : <p role="status">No rankings available for this selection.</p>}
      <p className="cfb-footnote">Quality wins are wins over model-rated Top 25 opponents. Logos show up to three strongest opponents beaten and three weakest opponents lost to, ordered by opponent-adjusted strength in the selected resume. Projected mode includes simulated results. Average records use one decimal; win/loss logos are shown only for individual projections because opponents beaten vary across simulations.</p>
      {basis === 'projected' && <details className="ap-details"><summary>Game assumptions ({projected.assumptions.length})</summary><div className="ap-assumptions">{projected.assumptions.map(g => <div key={g.id}><span>{g.away} at {g.home}</span><strong>{projected.averaged ? `${g.home} wins ${(g.homeWinFrequency * 100).toFixed(0)}% of projections` : `${g.winner} by ${g.margin.toFixed(1)}`}{g.forced ? ' · Forced' : ''}</strong></div>)}</div></details>}
    </>}
