...
from ap_preseason import FEATURES,HISTORY,load_inputs
ROOT=Path(__file__).parent

def evaluate(model,o):
    order=np.argsort(-predict(model,o['x'],feature_names=FEATURES),kind='stable')
...
def main():
    ...
    result={'schemaVersion':1,'season':year,'releaseDate':dates[str(year)],'generatedAt':now.isoformat(),
        ...
        'snapshot':{'season':year,'x':target['x'].tolist(),'teams':target['teams']} if target else None}
