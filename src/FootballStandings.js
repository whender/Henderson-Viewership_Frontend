...
export default function FootballStandings({ data }) {
  ...
  return <section className="home-panel cfb-standings">
    ...
    <p className="cfb-footnote">{projected ? 'Average projected final regular-season records: completed results plus each remaining game’s win probability, including games in progress. This is the exact simulation-average expectation, displayed to one decimal, without random sampling noise.' : 'Completed regular-season games only. Games in progress count once final.'} Ordered by conference winning percentage; tied percentages share a position. Official conference tiebreakers are not applied. Independents are ordered by overall winning percentage.</p>
    ...
    {visible.map(group => <section key={group.conference} className="cfb-standings-conference" aria-label={group.conference}>
      ...
      <table className="cfb-table"><caption className="sr-only">{projected ? 'Predicted' : 'Current'} {group.conference} standings</caption><thead><tr><th scope="col">Pos.</th><th scope="col">Team</th><th scope="col">Conf.</th><th scope="col">Overall</th></tr></thead><tbody>{group.rows.map(team => <tr key={team.team_id}>
        ...
        <td className="cfb-number">{group.isIndependent ? '—' : recordText(team.league, projected)}</td><td className="cfb-number">{recordText(team.overall, projected)}</td>
      </tr>)}</tbody></table>
    </section>)}
