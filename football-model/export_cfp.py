...
def main():
    ...
    out={'schemaVersion':1,'asOf':now.isoformat(),'season':year,'firstRelease':dates[0],'schedule':dates,'trainingThrough':validation['trainingThrough'],
        'trainingPolls':validation['trainingPolls'],'archivePolls':len(polls),'next':{'releaseDate':next_date,'averaged':bool(engine),'count':25,'rows':outlook,'alternatives':variants,'assumptions':assumptions,'unprojectedGames':len(missing),'scenarioEngine':engine,'eligibleTeams':scenarios['eligibleTeams'],'simulation':{'count':1000,'projections':25,'seed':SIMULATION_SEED,'method':'Default ranks and records average 25 seeded independent outcome projections. Override eligibility uses next-release Top 25 frequency across 1,000 simulations.'}},
        ...
        'methodology':['Predicts committee Top 25 order, not playoff seeds or qualification probabilities.',
            ...
            'The default view averages ranks, wins, losses and quality wins across 25 seeded projections using current football-model probabilities; teams are ordered by average rank. Browse the same 25 individual projections with whole-number records. Overrides are restricted to games involving the 25 teams most frequently ranked in the next-release Top 25 across 1,000 unmodified simulations. Each underlying scenario rebuilds opponent-adjusted strength and committee resumes with whole-number records before averaging. Hypothetical margin magnitudes come from the football model; in-progress games use the saved pregame probability, not live win odds.'],
